...
import { PICKLEBALL_CATEGORIES } from "../../data/pickleballChampionship";
...
function categoryLabel(id) {
  return PICKLEBALL_CATEGORIES.find((c) => c.id === id)?.label ?? id;
}

/**
 * Admin — Pickleball championship registrations.
 */
export default function AdminPickleball() {
    ...
    try {
      ...
      const statusParam =
        statusFilter && statusFilter !== "all"
          ...
          : "";
      const res = await request(`/api/admin/pickleball?${q}${statusParam}`);
  ...
  const loadSettings = useCallback(async () => {
    try {
      const res = await request("/api/admin/pickleball/settings");
  ...
  async function handleExport() {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/admin/pickleball/export`, {
      ...
      a.href = url;
      a.download = "evolve-pickleball-registrations.csv";
      a.click();
  ...
  return (
    <AdminLayout title="Pickleball championship">
      <SEO
        title="Pickleball registrations"
        description="Evolve Fitness admin — pickleball championship."
      ...
      <div className="admin-member-toolbar" style={{ marginBottom: "1.25rem" }}>
        <label className="admin-member-toolbar__label" htmlFor="pk-status">
          Status
        </label>
        <select
          id="pk-status"
